...
class HttpRequest:
    ...
    def get(self, url: str, id: str, token: str, connectionTimeout: int = 5, readTimeout: int = 3):
        ...
        return self.requestParseResult(response)
    ...
    #
    #-------------------------------------------------------------------------------------------------------------------
    def postToken(self, url: str, apiKey: str, data, connectionTimeout: int = 5, readTimeout: int = 3):
    ...
    #
    #-------------------------------------------------------------------------------------------------------------------
    def requestParseResult(self, response):
        myUtils = Utils()
        ...
        try:
            ...
            if not self.quiet:
                ...
                if myUtils.isJson(response.text) and 'message' in response.json():
            ...
            if myUtils.isJson(response.text) and 'results' in response.json():
            ...
            if myUtils.isJson(response.text) and 'message_error' in response.json():
